...
let rec compile_file (options : compile_options) : unit =
  ...
  try
    ...
    if options.verbose then Printf.printf "Phase 5: Code generation...\n";
    let output_filename = match options.output_file with
      ...
      | None -> 
          ...
          base ^ ".c"
    in
    if options.debug then Printf.printf "Writing output to: %s\n" output_filename;
    
    (* Create a simple placeholder output *)
    let oc = open_out output_filename in
    Printf.fprintf oc "/* Generated by Cresta Compiler */\n";
    Printf.fprintf oc "/* Source file: %s */\n\n" options.input_file;
    Printf.fprintf oc "#include <stdio.h>\n\n";
    Printf.fprintf oc "int main() {\n";
    Printf.fprintf oc "    printf(\"Hello from Cresta!\\n\");\n";
